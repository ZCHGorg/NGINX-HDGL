...
class HDGLDNSHandler(socketserver.BaseRequestHandler):
    ...
    lattice    = None
    domain_map: Dict[str, dict] = {}   # {"app.example.com": {"port":8080}, ...}
...
class HDGLResolver:
    ...
    e.g. {"app.example.com": {"port": 8080}, "site.example.com": {"port": 8081}}
...
if __name__ == "__main__":
    ...
    lat = HDGLLattice()
    lat.update("10.0.0.10", 45.0, 120.0)
    lat.update("10.0.0.11", 62.0,  80.0)

    domain_map = {
        "app.example.com":     {"port": 8080},
        "site.example.com":    {"port": 8081},
        "docs.example.com":    {"port": 8082},
        "cluster.example.com": {"port": 443},
    }

    resolver = HDGLResolver(lat, domain_map,
                            local_node="10.0.0.10",
    ...
    for domain in domain_map:
        ...
        expected = top.get(strand, ("10.0.0.10", 0))[0]
    ...
    # Test www. prefix
    ip_www = query("www.app.example.com")
    ip_bare = query("app.example.com")
    ok = "✓" if ip_www == ip_bare else "✗"
    if ip_www != ip_bare:
        all_pass = False
    print(f"  {ok} www. prefix stripped:  www.app.example.com → {ip_www} == app.example.com → {ip_bare}")
