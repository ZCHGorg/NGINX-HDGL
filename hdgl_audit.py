...
def t_strand_bounds():
    paths = ["/","/a","/a/b","/a/b/c/d/e","/service/x","/api/v1/users"]
...
def t_fileswap_matrix():
    swap = make_swap()
    swap.write("/service/config.json", b"{}")
...
def t_integration():
    ...
    files = {
        "/service/config.json": b'{"svc":"service"}',
        "/web/index.html":      b"<html>content</html>",
        "/data/users.json":     b'[{"id":1}]',
...
def t_moire_roundtrip():
    ...
    cases = [
        ("/service/config.json",     b'{"svc":"service"}'),
        ("/media/track01.mp3",       bytes(range(256)) * 4),
...
def t_moire_instance_isolation():
    ...
    paths = ["/netboot/alpine-A", "/netboot/alpine-B",
             "/netboot/staging-env", "/service/config.json"]
