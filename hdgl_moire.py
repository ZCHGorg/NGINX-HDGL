...
class HDGLMoire:
    ...
    def _phi_tau_standalone(path: str) -> float:
        """Standalone phi_tau — exact match to hdgl_fileswap._phi_tau algorithm."""
        segments = [s for s in path.strip("/").split("/") if s]
        tau = 0.0
        for depth, seg in enumerate(segments):
            # Intra-segment hash: plain sum of ord values, normalized (matches fileswap)
...
if __name__ == "__main__":
    ...
    test_cases = [
        ...
        ("/service/config.json",   b'{"service":"service","port":8080}'),
