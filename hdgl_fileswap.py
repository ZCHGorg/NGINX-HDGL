...
Usage:
    ...
    lattice = HDGLLattice()
    swap    = HDGLFileswap(lattice, local_node="10.0.0.10")

    swap.write("/app/config.json", b"{...}")
    data = swap.read("/app/config.json")
...
PASSIVE_SWAP_MAX_GB = float(os.getenv("LN_PASSIVE_SWAP_MAX_GB", "7"))
PASSIVE_SWAP_TIDY_EVERY = max(1, int(os.getenv("LN_PASSIVE_SWAP_TIDY_EVERY", "64")))
...
def _normalize_swap_path(path: str) -> str:
    """
    Canonicalize logical fileswap paths.

    Transport URLs use /swap as endpoint prefix; logical route paths must not.
    This strips any leading /swap prefixes to prevent recursive growth like
    /swap/swap/.../service/config.json.
    """
    p = (path or "").strip()
    if not p:
        return "/"
    ...
    while p == "/swap" or p.startswith("/swap/"):
        p = p[5:] if p.startswith("/swap/") else "/"
        if not p:
            p = "/"
...
def _fetch_from(node: str, swap_path: str) -> Optional[bytes]:
    ...
    canon_path = _normalize_swap_path(swap_path)
    if canon_path == "/":
        return None
    url = f"http://{node}:{SWAP_HTTP_PORT}/swap{canon_path}"
    try:
        r = requests.get(url, timeout=5)
        if r.status_code == 200:
            return r.content
    except requests.RequestException as e:
        log.debug(f"Fetch from {node} failed: {e}")
    return None


def _is_valid_authority_ip(ip: str) -> bool:
    """True for routable unicast authority addresses used for fetch/migrate."""
    try:
...
class HDGLFileswap:
    ...
    def __init__(self, lattice: Any, local_node: str) -> None:
        ...
        self._passive_root = SWAP_ROOT / "passive"
        self._passive_ops  = 0
        self._passive_limit_bytes = max(0, int(PASSIVE_SWAP_MAX_GB * (1024 ** 3)))

        SWAP_ROOT.mkdir(parents=True, exist_ok=True)
        self._passive_root.mkdir(parents=True, exist_ok=True)
    ...
    def write(self, path: str, data: bytes,
        ...
        """
        path = _normalize_swap_path(path)
    ...
    def read(self, path: str) -> Optional[bytes]:
        ...
        if authority == self.local_node and local_path.exists():
            ...
            log.debug(f"[fileswap] local authority read {path}")
            return data

        # Remote fetch
        log.debug(f"[fileswap] fetching {path} from {authority}")
        ...
        if raw_remote:
            ...
            self.record_passive_fetch(path, data, authority)
            return data
        ...
        if echo_entry and not echo_entry.expired():
            log.debug(f"[fileswap] echo fallback hit for {path} from mirror {echo_entry.authority}")
            return echo_entry.data

        mirror = self._mirror_for(strand_idx)
        if mirror != authority:
            log.debug(f"[fileswap] trying mirror node {mirror} for {path}")
            ...
            if data:
                ...
                self.record_passive_fetch(path, data, mirror)
                log.debug(f"[fileswap] served from mirror {mirror}")
                return data
    ...
    def passive_status(self) -> Dict[str, Any]:
        """
        Lightweight passive mirror telemetry for node_info.

        Lets operators track natural warm-up progress on non-dominant nodes
        without scanning filesystem state manually.
        """
        usage = self._passive_usage_bytes()
        limit = self._passive_limit_bytes
        pct = (usage / limit * 100.0) if limit > 0 else 0.0
        return {
            "enabled": PASSIVE_SWAP_ENABLE,
            "mode": "per-request-authority",
            "capture_when": "source_authority != local_node",
            "non_dominant": self._is_non_dominant(),
            "owned_strands": self._dominance_owned_strands(),
            "path": str(self._passive_root),
            "usage_bytes": usage,
            "usage_gb": round(usage / (1024 ** 3), 3),
            "limit_bytes": limit,
            "limit_gb": round(limit / (1024 ** 3), 3) if limit > 0 else 0.0,
            "usage_pct": round(pct, 2),
            "ops": self._passive_ops,
            "tidy_every_ops": PASSIVE_SWAP_TIDY_EVERY,
        }

    def record_passive_fetch(self, path: str, data: bytes,
                             source_authority: str) -> None:
        """
        Persist passively fetched content when this node is not the authority
        for that specific request.

        This is per-request dominance (case-by-case), not global node dominance.
        """
        if not PASSIVE_SWAP_ENABLE:
            return
        path = _normalize_swap_path(path)
        if path == "/":
            return
        if source_authority == self.local_node:
            return
        self._persist_passive(path, data)
    ...
    def _dominance_owned_strands(self) -> int:
        """Return how many strands this node currently owns."""
        top_map = self.lattice.top_node_per_strand()
        return sum(1 for _, (node, _) in top_map.items() if node == self.local_node)

    def _is_non_dominant(self) -> bool:
        """Treat nodes owning < half of strands as non-dominant for passive warm-up."""
        return self._dominance_owned_strands() < (NUM_STRANDS // 2)

    def _passive_path_for(self, path: str) -> Path:
        """Map logical file path to passive swap mirror path."""
        return self._passive_root / path.lstrip("/")

    def _passive_usage_bytes(self) -> int:
        total = 0
        try:
            for f in self._passive_root.rglob("*"):
                if f.is_file():
    ...
    def _persist_passive(self, path: str, data: bytes) -> None:
        """
        Passively mirror user-visited content into swap/passive.

        Called only through record_passive_fetch(), which enforces per-request
        authority checks (source_authority != local_node).
        """
        if not PASSIVE_SWAP_ENABLE:
            return
        if self._passive_limit_bytes <= 0:
            return
        try:
            p = self._passive_path_for(path)
            p.parent.mkdir(parents=True, exist_ok=True)
            p.write_bytes(data)
            self._passive_ops += 1
            if self._passive_ops % PASSIVE_SWAP_TIDY_EVERY == 0:
                self._tidy_passive_swap()
        except OSError as e:
            log.debug(f"[fileswap] passive persist skipped for {path}: {e}")

    def _tidy_passive_swap(self) -> None:
        """
        Enforce passive mirror disk budget with LRU-atime eviction.

        Target after tidy is 95% of budget to avoid constant churn near the cap.
        """
        if not PASSIVE_SWAP_ENABLE or self._passive_limit_bytes <= 0:
            return
        usage = self._passive_usage_bytes()
        if usage <= self._passive_limit_bytes:
            return

        target = int(self._passive_limit_bytes * 0.95)
        files = []
        for f in self._passive_root.rglob("*"):
            if not f.is_file():
                continue
        ...
        if removed:
            log.info(
                f"[fileswap] passive tidy removed={removed} "
                f"freed={freed/1e6:.1f}MB usage={usage/1e6:.1f}MB"
            )
        ...
        if bin_path.exists():
            try:
                ...
                for path, r in raw_bin.items():
                    canon_path = _normalize_swap_path(path)
                    if not _is_valid_authority_ip(r.get("authority", "")):
                        r["authority"] = self._authority_for(r["strand_idx"])
                        cleaned += 1
                    if canon_path != path:
                        cleaned += 1
                    r["path"] = canon_path
                    self._routes[canon_path] = SwapRoute(**r)
                log.info(
                    f"[fileswap] loaded {len(self._routes)} routes from binary "                    f"({bin_path.stat().st_size} bytes)"                )
                if cleaned:
                    log.warning(
                        f"[fileswap] sanitized {cleaned} invalid route authority address(es)"
                    )
                    self._save_routes()
                return
            except Exception as e:
...
if __name__ == "__main__":
    ...
    lattice = HDGLLattice()
    nodes   = [
        {"node": "10.0.0.10", "latency": 50,  "storage_avail_gb": 1.0},
        {"node": "10.0.0.11", "latency": 80,  "storage_avail_gb": 2.0},
    ]
    for n in nodes:
