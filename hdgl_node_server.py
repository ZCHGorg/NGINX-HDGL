...
def _detect_outbound_ip() -> str:
    """Return the IP of the outbound interface, avoiding 127.0.1.1 on Ubuntu/Debian."""
    try:
...
def _is_valid_cluster_ip(ip: str) -> bool:
    """Return True for routable unicast cluster addresses used in gossip."""
    try:
...
def _resolve_local_node() -> str:
    """
    Resolve node identity IP safely.

    Reject invalid LN_LOCAL_NODE values (loopback, etc.) and fall back to
    outbound interface detection to keep gossip/node_info routable.
    """
...
SERVE_FAIL_LOG_EVERY = max(1, int(os.getenv("LN_SERVE_FAIL_LOG_EVERY", "100")))
PHI           = (1 + math.sqrt(5)) / 2
...
class HDGLNodeHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            type(self).serve_failures += 1
            type(self).serve_last_error = f"unhandled:{type(e).__name__}:{e}"
            log.error(f"[serve] unhandled GET error for {self.path}: {e}", exc_info=True)
            try:
                self._send(500, {"error": "internal error", "path": self.path})
            except Exception:
                pass
    ...
    def _handle_node_info(self):
        ...
        try:
            passive = self.swap.passive_status()
        except Exception:
            passive = {}
        ...
        self._send(200, {
            ...
            "known_nodes":         [
                n for n in self.lattice._states.keys()
                if _is_valid_cluster_ip(n)
            ],
            "authority_strands":   [
                k for k, (n, _) in top.items() if n == NODE_IP
            ],
            "serve_stats": {
                "local_hits": type(self).serve_local_hits,
                "proxy_hits": type(self).serve_proxy_hits,
                "mirror_hits": type(self).serve_mirror_hits,
                "stale_hits": type(self).serve_stale_hits,
                "failures": type(self).serve_failures,
                "last_error": type(self).serve_last_error,
            },
            "passive_swap":        passive,
            "uptime_s":            round(time.time() - self.started, 1),
        })
    ...
    def _handle_serve(self, file_path: str):
        ...
        # Transport endpoint normalization:
        # /swap/<logical-path> is HTTP transport, not part of logical fileswap path.
        # Strip it here to prevent recursive growth (/swap/swap/...).
        while file_path == "/swap" or file_path.startswith("/swap/"):
            file_path = file_path[5:] if file_path.startswith("/swap/") else "/"
        ...
        # file_path is decoded for local path handling; re-encode for HTTP forwarding.
        encoded_path = quote(file_path, safe="/")
        proxy_url = f"http://{authority}:{NODE_PORT}/serve{encoded_path}"
        ...
        try:
            ...
            # Per-request passive capture: this node fetched from remote authority.
            self.swap.record_passive_fetch(file_path, data, authority)
            type(self).serve_proxy_hits += 1
            ...
            if isinstance(e, HTTPError):
                type(self).serve_last_error = (
                    f"authority_http_{e.code}:{authority}:{file_path}"
                )
            elif isinstance(e, (URLError, TimeoutError, OSError)):
                type(self).serve_last_error = (
                    f"authority_unreachable:{authority}:{file_path}:{e}"
                )
            else:
                type(self).serve_last_error = (
                    f"authority_exception:{type(e).__name__}:{authority}:{file_path}:{e}"
                )
            # Authority unreachable — try mirror (echo fallback)
            mirror = self.swap._mirror_for(strand)
            if mirror != authority and mirror != NODE_IP:
                try:
                    mirror_url = f"http://{mirror}:{NODE_PORT}/serve{encoded_path}"
                    req  = Request(mirror_url, headers={"X-HDGL-Forwarded-By": NODE_IP})
                    resp = urlopen(req, timeout=PROXY_TIMEOUT)
                    data = resp.read()
                    self.swap.record_passive_fetch(file_path, data, mirror)
                    type(self).serve_mirror_hits += 1
            ...
            type(self).serve_failures += 1
            # Avoid log floods under repeated 404/timeout traffic.
            if type(self).serve_failures <= 5 or type(self).serve_failures % SERVE_FAIL_LOG_EVERY == 0:
                log.warning(
                    f"[serve] failures={type(self).serve_failures} "
                    f"path={file_path} last_error={type(self).serve_last_error}"
                )
            self._send(503, {
    ...
    def _handle_gossip(self, body: bytes):
        ...
        try:
            ...
            if not _is_valid_cluster_ip(nid):
                log.warning(f"[gossip/{protocol}] rejected invalid node address: {nid}")
                self._send(400, {"error": "invalid node identifier", "node": nid})
                return
